...
def main():
    ...
    #     sys.exit()
    for input_filename in os.listdir("./0"):
        tok = input_filename.replace(".", "_").split("_")
        if int(tok[1]) % 2 == 0:
            samrate, data = wavfile.read(str("./0/"+input_filename))
            print('Load is Done! \n')
            ...
            if len(wavData.columns) == 2:
                ...
                stereo_L.to_csv(str("./0_temp/"+input_filename[:-4] + "_temp.csv"), mode='w')
                # wavData.to_csv("Output_stereo_RL.csv", mode='w')
            elif len(wavData.columns) == 1:
                print('Mono .wav file found\n')
                wavData.columns = ['M']
                wavData.to_csv(str("./0_temp/"+input_filename[:-4] + "_temp.csv"), mode='w')
            else:
                print('Multi channel .wav file found\n')
                print('number of channel : ' + len(wavData.columns) + '\n')
                wavData.to_csv(str("./0_temp/"+input_filename[:-4] + "_temp.csv"), mode='w')
            reformat_csv(str("./0_temp/"+input_filename[:-4] + "_temp.csv"), str("snore-data-NoSnore-" + input_filename[:-4] + ".csv"))
            print("Saving done")
