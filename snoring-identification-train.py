...
data_dir = './data/data_even'  # directory where the data files are stored
...
# the filenames should be in the form 'snore-data-subject-1.csv'.
# 'snore-data-Snore-1.csv' 'snore-data-NoSnore-1.csv'
...
for i, window_with_timestamp_and_label in enumerate(data):
    ...
    if len([x for x in window if x != 0]) != 0:
        ...
        y = np.append(y, label)
    if i % 250 == 0 and i != 0:
        print("Processed features of",i,"windows")
...
for tree_in_forest in best_classifier.estimators_:
    export_graphviz(tree_in_forest, out_file='tree.dot', feature_names = ["mean", "num_peaks", "fft", "entropy"], class_names = class_names)

os.system('dot -Tpng tree.dot -o tree.png')
# export_graphviz(best_classifier, 
#                 out_file='tree.dot', 
#                 feature_names = ["mean", "num_peaks", "fft", "entropy"],
...
# export_graphviz(best_classifier, out_file='tree.dot', feature_names = y)
